        ...
        {items.map((item) => (
          <AccordionItem value={item.id} key={item.id} className="py-5">
            <AccordionPrimitive.Header className="flex">
              <AccordionPrimitive.Trigger className="flex flex-1 items-center justify-between py-2 text-left text-lg font-semibold leading-6 transition-all [&>svg>path:last-child]:origin-center [&>svg>path:last-child]:transition-all [&>svg>path:last-child]:duration-200 [&[data-state=open]>svg>path:last-child]:rotate-90 [&[data-state=open]>svg>path:last-child]:opacity-0 [&[data-state=open]>svg]:rotate-180">
                {item.title}
                <Plus
                  size={16}
                  strokeWidth={2}
            ...
            <AccordionContent className="pb-2 text-base pt-2 opacity-80 text-muted-foreground">
              {item.content}
            </AccordionContent>
          </AccordionItem>
        ))}
