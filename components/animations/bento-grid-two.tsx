...
const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-col md:grid w-full auto-rows-[22rem] grid-cols-3 gap-4",
        "grid-rows-[repeat(2,22rem)]", // Add this to enforce 2 rows
        className
      )}
    >
      {children}
...
const BentoCard = ({
  ...
  >
    ...
    <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-6 transition-all duration-300 group-hover:-translate-y-10">
      ...
      <h3 className="font-helvatica text-xl font-semibold text-neutral-300">
        {name}
      </h3>
      <p className="font-helvatica max-w-lg">{description}</p>
    </div>
    ...
    >
      <Button
        ...
        asChild
        size="sm"
        className="font-helvatica pointer-events-auto"
      >
        <a href={href}>
          {cta}
